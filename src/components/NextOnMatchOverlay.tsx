  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between gap-4 px-6 md:px-12 pt-5 shrink-0">
        ...
        <div className="flex items-center gap-2.5">
          <div className="hidden sm:block leading-tight text-right">
            ...
            <div className="text-[8px] text-white/35 tracking-[.15em] uppercase">{lang === 'ar' ? 'البث' : lang === 'fr' ? 'Diffusion' : 'Broadcast'}</div>
          </div>
          <div className="w-11 h-11 md:w-14 md:h-14 rounded-xl border border-[#f2c14e]/30 bg-black/30 flex items-center justify-center font-display font-black text-[#f2c14e]">TK</div>
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-wrap justify-center gap-2 px-6 mt-4 mb-4 shrink-0">
        {modeLabel && <StatBox icon={<Swords size={11} />} label={lang === 'ar' ? 'النوع' : lang === 'fr' ? 'Type' : 'Type'} value={modeLabel} delay={.12} />}
        <StatBox icon={<MapPin size={11} />} label={lang === 'ar' ? 'رقم البساط' : lang === 'fr' ? 'N° Tapis' : 'MAT NO.'} value={state.matNumber ? `MAT ${String(state.matNumber).padStart(2, '0')}` : 'MAT 01'} delay={.145} />
        ...
        {state.weightCategory && <StatBox icon={<Scale size={11} />} label={lang === 'ar' ? 'الوزن' : lang === 'fr' ? 'Poids' : 'Weight'} value={state.weightCategory} delay={.20} />}
        {ageLabel && <StatBox icon={<User size={11} />} label={lang === 'ar' ? 'العمر' : lang === 'fr' ? 'Âge' : 'Age'} value={ageLabel} delay={.22} />}
        {state.gender && <StatBox label={lang === 'ar' ? 'الجنس' : lang === 'fr' ? 'Genre' : 'Gender'} value={state.gender === 'male' ? (lang === 'ar' ? 'رجال' : lang === 'fr' ? 'HOMMES' : 'MEN') : (lang === 'ar' ? 'نساء' : lang === 'fr' ? 'FEMMES' : 'WOMEN')} delay={.25} />}
        {stageLabel && <StatBox icon={<Trophy size={11} />} label={lang === 'ar' ? 'المرحلة' : lang === 'fr' ? 'Phase' : 'Stage'} value={stageLabel} delay={.28} />}
      </div>
      ...
      <div className="relative z-10 flex-1 min-h-0 flex items-center justify-center gap-4 md:gap-8 px-4 md:px-10">
        ...
        <div className="flex flex-col items-center shrink-0 px-2 relative" style={{ order: 2, animation: 'nextCenterIn .7s ease-out both', animationDelay: '.45s' }}>
          ...
          <div className="font-display font-black tracking-[.35em] uppercase mt-2 whitespace-nowrap relative" style={{ fontSize: 'clamp(12px,1.4vw,18px)', color: '#f5d477', textShadow: '0 0 14px #f2c14ea6,0 0 30px #f2c14e4d' }}>
            {lang === 'ar' ? 'المباراة القادمة' : lang === 'fr' ? 'Prochain match' : 'Next On The Match'}
          </div>
      ...
      <div className="relative z-10 flex flex-wrap justify-center gap-2 px-6 py-4 shrink-0" style={{ borderTop: '1px solid rgba(75,82,105,.45)', background: 'rgba(10,13,22,.72)' }}>
        ...
        <StatBox icon={<ClipboardCheck size={11} />} label={lang === 'ar' ? 'حالة المباراة' : lang === 'fr' ? 'État du match' : 'Match Status'} value={lang === 'ar' ? 'جاهز' : lang === 'fr' ? 'PRÊT' : 'READY'} delay={.58} />
        <StatBox icon={<Clock size={11} />} label={lang === 'ar' ? 'وقت الجولة' : lang === 'fr' ? 'Temps de round' : 'Round Time'} value={formatTime(state.config.roundTime)} delay={.61} />
        <StatBox icon={<Cpu size={11} />} label={lang === 'ar' ? 'حالة النظام' : lang === 'fr' ? 'État système' : 'System Status'} value={lang === 'ar' ? 'يعمل' : lang === 'fr' ? 'OPÉRATIONNEL' : 'OPERATIONAL'} connection delay={.64} />
        <StatBox icon={<ShieldCheck size={11} />} label={lang === 'ar' ? 'الحكم متصل' : lang === 'fr' ? 'Arbitre connecté' : 'Referee Connected'} value={lang === 'ar' ? 'نعم' : lang === 'fr' ? 'OUI' : 'YES'} connection delay={.67} />
      </div>
