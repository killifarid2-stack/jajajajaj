...
export default function OperatorScreen() {
  ...
  const handleIVRDecision = (decision: 'accept' | 'reject') => {
    ...
    // Persistent by request: the IVR/camera-review result now stays on the
    // public screen until the referee explicitly dismisses it (HIDE
    // RESULT button below), instead of auto-clearing after 3s.
  ...
  const handleNextMatch = async () => {
    ...
    if (tournamentId) {
      try {
        ...
        nextBracketMatch = bracket?.find((m: any) => m.id !== currentBracketMatchId && !m.winner && !m.isBye && m.player1 && m.player2) ?? null;
      } catch { /* fall through to the manual Quick Match form below */ }
    }
      ...
      {showMiniPreview && (
        <div
          ...
          style={previewPos
            ...
            : { width: previewWidth, bottom: '1rem', right: '1rem' }}
        >
          <div
            onMouseDown={(e) => {
          ...
          >
            <span className="text-[10px] font-display font-bold text-[hsl(var(--info))] tracking-wider flex items-center gap-1">
              <Eye size={11} /> LIVE PREVIEW — {previewTab === 'public' ? 'PUBLIC SCREEN' : 'JUDGE SCREEN'} · READ ONLY
            </span>
