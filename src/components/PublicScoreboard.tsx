        ...
        <div className="flex-1 min-h-0 grid grid-cols-[minmax(0,1fr)_minmax(250px,.78fr)_minmax(0,1fr)] gap-3 items-stretch">
          ...
          <section className="relative min-w-0 rounded-2xl overflow-hidden flex flex-col items-center justify-center" style={{ border: '3px solid rgba(255,216,102,.65)', background: 'linear-gradient(160deg, rgba(255,202,64,.12), rgba(2,6,13,.88) 42%, rgba(0,0,0,.97))', boxShadow: '0 0 48px rgba(255,194,35,.22), inset 0 1px 0 rgba(255,255,255,.1), inset 0 0 40px rgba(255,214,90,.06)' }}>
            ...
            <div className="relative z-20 -mt-3 rounded-xl px-5 py-2 text-center" style={{ border: '2px solid rgba(255,216,102,.7)', background: 'rgba(2,5,10,.9)', boxShadow: '0 0 32px rgba(255,190,30,.22)' }}>
              <div className="font-display font-bold text-[#ffd866] tracking-[.16em]" style={{ fontSize: 9 }} >ROUNDS WON</div>
              <div className="font-display font-black leading-none" style={{ fontSize: 'clamp(34px, 4vw, 58px)' }}><span style={{ color: red.color }}>{red.roundsWon}</span><span className="text-white/35 mx-2">:</span><span style={{ color: blue.color }}>{blue.roundsWon}</span></div>
              <div className="font-display text-white/35 tracking-[.14em] mt-0.5" style={{ fontSize: 7 }}>WINS</div>
            </div>
          </section>
...
function ScoreboardView({ isMiniPreview = false }: { isMiniPreview?: boolean } = {}) {
  ...
  // Persistent by request: this used to auto-hide after 4.6s, but the
  // operator wants IVR/camera-review results to stay on screen until
  // explicitly dismissed (see handleIVRDecision / CLEAR_IVR_ANIMATE on the
  // Operator side), matching AI-analysis and match-end overlays.
  ...
  if (currentFrame === 'match_end' && result) {
    ...
    return (
      <div key="frame-end" className={`fixed inset-0 flex flex-col overflow-hidden ${isMiniPreview ? 'public-scoreboard-mini' : ''}`}
        ...
        <div className="text-center py-2 relative z-20 flex flex-col items-center gap-1" style={{ background: 'hsl(224 35% 8% / 0.9)', borderBottom: '1px solid hsl(224 35% 15%)' }}>
          ...
          <div className="flex items-center gap-4 flex-wrap justify-center">
            {[
              ...
              <div key={label} className="text-center">
                <div className="text-white/30 font-display tracking-[.12em]" style={{ fontSize: 7 }}>{label}</div>
                <div className="font-display font-black text-white/75" style={{ fontSize: 10 }}>{value}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 px-6 relative z-20">
          ...
          {/* Round Score — plain big numbers side by side, flat (no boxes),
              ...
              These are already ROUNDS WON (chungRoundWins/hongRoundWins,
              counted from the real per-round record), not raw points — the
              caption below makes that explicit instead of leaving two bare
              numbers open to misreading as a point score. */}
          <div className="text-center -mb-2" style={{ opacity: winnerScale ? 1 : 0, transition: 'opacity .6s ease-out .35s' }}>
            <div className="font-display font-black tracking-[.32em] text-[#ffd866]/80" style={{ fontSize: 11 }}>ROUNDS WON</div>
          </div>
          <div className="flex items-center gap-10" style={{
            ...
            <div className="text-center">
              ...
              {!isTeamReveal && <div className="font-display text-white/35 tracking-[.2em] -mt-1" style={{ fontSize: 10 }}>WINS</div>}
            </div>
            <div className="text-center">
              <div className="font-display font-black leading-none" style={{
                fontSize: 'clamp(60px, 12vw, 120px)',
          ...
          {/* Round-by-round breakdown — points still shown here (per
              request: "النقاط لا تختفي، لكنها لا تكون هي نتيجة الفوز
              الرئيسية"), but the row count comes straight from the match's
              actual configured/played rounds (state.config.rounds /
              state.roundWinners), never a hardcoded 4 — a Best-of-3 match
              shows exactly 3 rows, and a real 4th (Golden) round only
              appears if it was actually played. */}
          {!isTeamReveal && (
            <div className="rounded-xl px-4 py-2" style={{ border: '1px solid rgba(255,255,255,.12)', background: 'rgba(0,0,0,.5)', opacity: winnerScale ? 1 : 0, transition: 'opacity .6s ease-out .5s' }}>
              <div className="text-center font-display text-white/40 tracking-[.24em]" style={{ fontSize: 9 }}>ROUND RESULTS</div>
              <div className="mt-1.5 flex flex-col gap-1">
                {Array.from({ length: Math.max(state.config?.rounds || 3, state.roundWinners?.length || 0) }, (_, idx) => idx + 1).map(rn => {
          ...
          {/* Gold-framed WINNER badge — a distinct laurel-style label above
              the champion bar, matching the reference graphic's golden
              "WINNER" frame instead of only the plain "MATCH X RESULT"
              header higher up the screen. */}
          <div className="text-center" style={{ opacity: winnerScale ? 1 : 0, transform: winnerScale ? 'scale(1)' : 'scale(.85)', transition: 'all 0.6s cubic-bezier(.2,.8,.25,1) .55s' }}>
            <div className="inline-block rounded-2xl px-8 py-1.5" style={{
              border: '2px solid hsl(45 93% 58% / 0.85)',
              background: 'linear-gradient(180deg, hsl(45 93% 58% / .16), transparent)',
              boxShadow: '0 0 40px hsl(45 93% 58% / .3), inset 0 0 24px hsl(45 93% 58% / .12)',
            }}>
              <div className="font-display font-black tracking-[.22em]" style={{ fontSize: 'clamp(22px,3vw,38px)', color: 'hsl(45 93% 62%)', textShadow: '0 0 24px hsl(45 93% 58% / .55)' }}>
                {t('broadcastWinner')}
              </div>
            </div>
          </div>
        ...
        <div className="relative z-10 flex-1 min-h-0 px-4 py-3">
          <div className="mx-auto max-w-[1900px] h-full grid grid-cols-[minmax(220px,1fr)_minmax(300px,520px)_minmax(220px,1fr)] gap-4 items-stretch">
            <div className="relative overflow-hidden rounded-2xl border-2 border-red-500/70" style={{ background: 'radial-gradient(circle at 50% 48%, rgba(255,42,62,.24), transparent 48%), linear-gradient(160deg, rgba(75,5,12,.95), rgba(7,3,7,.98))', boxShadow: '0 0 45px rgba(255,42,62,.16), inset 0 0 45px rgba(255,42,62,.08)' }}>
              ...
              <div className="absolute inset-0 flex items-end justify-center pt-7 pb-[172px]">
              ...
              <div className="absolute left-3 top-11 z-10 rounded-xl border border-white/10 bg-black/60 px-2.5 py-2 shadow-[0_0_22px_rgba(0,0,0,.35)]">
                {bestOfRound === 'hong' && <div className="flex items-center gap-1 text-[8px] font-display font-black tracking-[.1em] text-[#ffd866] mb-1"><Star size={9} fill="currentColor"/> BEST OF R{currentRound}</div>}
                <div className="grid grid-cols-3 gap-1.5 items-end">
                  {[1, 2, 3].map(rn => (
                    <div key={rn} className={`min-w-[38px] rounded-md border px-1.5 py-1 text-center ${rn === 3 ? 'border-red-300/55 bg-red-500/15' : 'border-white/10 bg-white/[.025]'}`}>
                      <div className={`text-[8px] font-display font-black tracking-[.08em] ${rn === 3 ? 'text-red-200' : 'text-white/40'}`}>R{rn}</div>
                      <div className={`font-display font-black tabular-nums leading-none mt-0.5 ${rn === 3 ? 'text-lg text-white' : 'text-sm text-white/85'}`}>{roundScoreCell('hong', rn)}</div>
                    </div>
                  ))}
                </div>
                <div className="mt-1.5 rounded-md border border-[#ffd866]/35 bg-[#ffd866]/10 px-2 py-1 text-center">
                  <div className="text-[7px] font-display font-black tracking-[.14em] text-[#ffd866]/70">TOTAL</div>
                  <div className="font-display font-black text-xl leading-none text-[#ffd866] tabular-nums">{state.hong.totalScore ?? 0}</div>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-white/10 flex flex-col items-center justify-center px-4 py-3" style={{ background: 'radial-gradient(circle at 50% 45%, rgba(255,216,102,.10), transparent 45%), rgba(0,0,0,.72)', boxShadow: '0 0 55px rgba(0,0,0,.55), inset 0 0 30px rgba(255,255,255,.025)' }}>
              ...
              {pointGapWatch && pointGapLeader && (
                <div className="mt-3 rounded-xl border-2 border-orange-400/55 bg-orange-500/[.10] px-4 py-2.5 text-center max-w-full shadow-[0_0_24px_rgba(255,150,40,.15)]">
                  <div className="font-display text-[9px] tracking-[.25em] text-orange-300/85 flex items-center justify-center gap-1.5"><AlertTriangle size={11}/> {ui('POINT GAP WATCH','مراقبة فارق النقاط','SURVEILLANCE DE L’ÉCART')}</div>
                  <div className="font-display font-black text-sm mt-1">
                    <span className={pointGapLeader === 'chung' ? 'text-[hsl(var(--chung))]' : 'text-[hsl(var(--hong))]'}>{pointGapLeader === 'chung' ? 'BLUE' : 'RED'}</span>
                    {ui(` LEADS BY ${liveGap} (THRESHOLD ${pointGapThreshold})`,` متقدّم بفارق ${liveGap} (الحد ${pointGapThreshold})`,` MÈNE DE ${liveGap} (SEUIL ${pointGapThreshold})`)}
                  </div>
                  <div className="mt-0.5 text-[8px] text-white/50 tracking-[.1em]">{ui('NEXT ROUND MAY END EARLY ON PTG','الجولة القادمة قد تُحسم مبكرًا بفارق النقاط','LA PROCHAINE MANCHE PEUT SE TERMINER TÔT PAR ÉCART')}</div>
                </div>
              )}
              {finalRoundAlert && (
            ...
            <div className="relative overflow-hidden rounded-2xl border-2 border-blue-500/70" style={{ background: 'radial-gradient(circle at 50% 48%, rgba(32,130,255,.24), transparent 48%), linear-gradient(160deg, rgba(5,28,90,.98), rgba(3,5,12,.98))', boxShadow: '0 0 45px rgba(32,130,255,.16), inset 0 0 45px rgba(32,130,255,.08)' }}>
              ...
              <div className="absolute right-3 top-11 z-10 rounded-xl border border-white/10 bg-black/60 px-2.5 py-2 shadow-[0_0_22px_rgba(0,0,0,.35)]">
                {bestOfRound === 'chung' && <div className="flex items-center gap-1 text-[8px] font-display font-black tracking-[.1em] text-[#ffd866] mb-1"><Star size={9} fill="currentColor"/> BEST OF R{currentRound}</div>}
                <div className="grid grid-cols-3 gap-1.5 items-end">
                  {[1, 2, 3].map(rn => (
                    <div key={rn} className={`min-w-[38px] rounded-md border px-1.5 py-1 text-center ${rn === 3 ? 'border-blue-300/55 bg-blue-500/15' : 'border-white/10 bg-white/[.025]'}`}>
                      <div className={`text-[8px] font-display font-black tracking-[.08em] ${rn === 3 ? 'text-blue-200' : 'text-white/40'}`}>R{rn}</div>
                      <div className={`font-display font-black tabular-nums leading-none mt-0.5 ${rn === 3 ? 'text-lg text-white' : 'text-sm text-white/85'}`}>{roundScoreCell('chung', rn)}</div>
                    </div>
                  ))}
                </div>
                <div className="mt-1.5 rounded-md border border-[#ffd866]/35 bg-[#ffd866]/10 px-2 py-1 text-center">
                  <div className="text-[7px] font-display font-black tracking-[.14em] text-[#ffd866]/70">TOTAL</div>
                  <div className="font-display font-black text-xl leading-none text-[#ffd866] tabular-nums">{state.chung.totalScore ?? 0}</div>
                </div>
              </div>
              <div className="absolute bottom-3 left-3 right-3 z-10 text-center max-h-[164px] overflow-hidden">
    ...
    <div dir="ltr" className="fixed inset-0 z-[220] flex items-center justify-center bg-black/92 backdrop-blur-sm pointer-events-none">
      <div className="w-[min(94vw,1200px)] rounded-3xl border-2 border-[hsl(var(--gold))]/60 bg-[#070a10] shadow-[0_0_80px_rgba(245,200,66,.16)] overflow-hidden">
        ...
        <div className="p-8 min-h-[420px] relative">
          ...
          {tieReview?.phase === 'countdown' && <div data-wab-layer-root="woose-girok" data-wab-layer-id="woose-girok-root" className="h-full flex flex-col items-center justify-center text-center relative overflow-hidden girok-public-scene"><img data-wab-layer-id="woose-girok-arms" src={wooseGirokArmsUrl} alt="" className="girok-public-arms countdown-arms"/><div data-wab-layer-id="woose-girok-countdown" className="relative z-10 girok-count-between"><div data-wab-layer-id="woose-girok-count" className="text-[hsl(var(--gold))] text-8xl font-display font-black">{tieReview.countdownStep}</div><div data-wab-layer-id="woose-girok-count-label" className="text-2xl font-black text-white mt-2">{tieReview.countdownStep===1?'HANA — 하나':tieReview.countdownStep===2?'DUL — 둘':'SET — 셋'}</div></div></div>}
          ...
          {tieReview?.phase === 'result' && (() => { const blue=Object.values(tieReview.votes).filter(v=>v==='chung').length; const red=Object.values(tieReview.votes).filter(v=>v==='hong').length; const winner=blue>=2?'chung':'hong'; const unanimous=blue===3||red===3; return <div data-wab-layer-root="woose-girok" data-wab-layer-id="woose-girok-result-root" className="h-full flex flex-col items-center justify-center text-center relative overflow-hidden girok-result-scene"><div className="relative z-10 w-full"><div className={`girok-decision-frame mx-auto relative overflow-hidden p-8 ${winner==='chung'?'is-chung':'is-hong'}`} style={{maxWidth:860}}><img src={winner==='chung'?judgeDecisionBlueArmUrl:judgeDecisionRedArmUrl} alt="" className="girok-decision-arm absolute inset-0 w-full h-full object-contain opacity-80 pointer-events-none" /><div className="relative z-10"><div className="girok-gold-title font-display text-xl md:text-2xl">{unanimous?'UNANIMOUS DECISION':'MAJORITY DECISION'}</div><div className={`mt-4 font-display font-black ${winner==='chung'?'text-[hsl(var(--chung))]':'text-[hsl(var(--hong))]'}`} style={{ fontSize: 'clamp(64px,9vw,140px)', lineHeight: 1, textShadow: `0 0 50px ${winner==='chung'?'hsl(var(--chung))':'hsl(var(--hong))'}66` }}>{winner==='chung'?'BLUE':'RED'}</div><div className="girok-result-frame mt-5"><div className="text-xs tracking-[.3em] text-white/50">ROUND {state.currentRound} WINNER</div><div className="text-4xl font-display font-black mt-2">{blue} — {red}</div><div className="text-sm mt-2 text-white/65">JUDGES DECISION</div></div><div className="mt-3 text-xs text-white/55 tracking-[.2em]">{state.status==='rest' ? `REST — ${formatTime(state.timeRemaining)}` : 'REST STARTING…'}</div></div></div><div className="text-[10px] tracking-[.3em] text-white/45 font-black mt-8 mb-4">THREE REFEREES · FINAL VOTE</div><div className="grid grid-cols-3 gap-4 max-w-3xl mx-auto">{(['left','center','right'] as const).map((id,i)=>{const vote=tieReview.votes[id]; const name=tieReview.judgeNames?.[id] || (i===0?'Judge 1':i===1?'Center Referee':'Judge 3'); const role=i===1?'CENTER / MAT REFEREE':'SIDE JUDGE'; const photo=tieReview.judgePhotos?.[id]; const armSrc = vote==='chung'?judgeDecisionBlueArmUrl:judgeDecisionRedArmUrl; return <div key={id} className={`girok-public-judge girok-frame-strong is-small ${i===1?'is-center':''} ${vote==='chung'?'is-voted-chung':'is-voted-hong'}`}><div className="girok-public-photo-wrap is-small">{photo ? <img src={photo} alt="" className="girok-public-photo"/> : <div className="girok-public-photo-placeholder">{name.slice(0,1).toUpperCase()}</div>}</div><img src={armSrc} alt="" className={`girok-judge-arms-voted is-small ${vote==='chung'?'is-chung-arm':'is-hong-arm'}`}/><div className="relative z-10"><div className="text-xs font-black">{name}</div><div className="text-[8px] tracking-[.16em] text-white/45 mt-1">{role}</div><div className={`mt-3 text-lg font-display font-black ${vote==='chung'?'text-[hsl(var(--chung))]':'text-[hsl(var(--hong))]'}`}>{vote==='chung'?'BLUE':'RED'}</div></div></div>})}</div></div></div> })()}
        </div>
      </div>
    </div>
  ) : null;
