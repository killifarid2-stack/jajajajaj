...
export function CenterMatch({ state, round, onSelectRound, onStartMatch, onConfirm, matchNumber, matchClock, totalRounds, frameOffsetX, frameOffsetY, frameScale, frameBorderColor, frameFrozen, titleColor }: Props) {
  const { t, lang } = useI18n();
  const c = getCopy(lang === "ar")[state];

  return (
    <section
      data-wab-layer-id="center"
    ...
    >
      ...
      <div data-wab-layer-id="match-number" className={`match-number hero ${c.tone}`}>
        <span className="mn-label">{t('match')}</span>
        <span className="mn-value">{matchNumber ?? '—'}</span>
      </div>

      <div data-wab-layer-id="clock" className="clock">
      ...
      <div data-wab-layer-id="call-card" className={`call-card ${c.tone}`}>
        <div className="call-title" style={titleColor ? { color: titleColor } : undefined}>
          {state === "ready" ? (
            <Check size={20} />
          ) : state === "confirm" ? (
            <Hourglass size={19} />
      ...
      <div
        data-wab-layer-id="trophy"
        className={`center-trophy hero ${c.tone}${frameFrozen ? ' wab-frame-frozen' : ''}`}
        style={{
          transform: `translate(${frameOffsetX ?? 0}px, ${frameOffsetY ?? 0}px) scale(${frameScale ?? 1})`,
