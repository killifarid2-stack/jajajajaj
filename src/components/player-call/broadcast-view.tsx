...
export function BroadcastView({ data, state, reducedMotion = false, qaMode = false, tournamentName, showReadyState = false }: Props) {
  ...
  return (
    <div
      data-wab-layer-id="player-call-root" data-wab-layer-root="player-call"
      className={`relative h-[1080px] w-[1920px] overflow-hidden bg-[#01030a] font-hud ${reducedMotion ? "pc-reduced" : ""}`}
    >
      <div data-wab-layer-id="player-call-background"><BroadcastBackground state={state} reducedMotion={reducedMotion} /></div>

      <div data-wab-layer-id="player-call-header"><ChampionshipHeader
      ...
      <div data-wab-layer-id="player-call-stage" className="absolute inset-x-0 top-[230px] bottom-[56px] z-20 grid grid-cols-[1fr_500px_1fr] gap-6 px-10">
