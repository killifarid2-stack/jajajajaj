...
export function PlayerCard({
  ...
  return (
    <motion.section
      ...
      data-wab-layer-id={`${side}-player-card`}
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Name banner */}
            <div data-wab-layer-id={`${side}-player-name`} className="relative mt-3">
              <div
              ...
              >
                <h2
                  className="px-4 font-display font-black uppercase leading-none tracking-[0.045em] text-[#f2c14e] whitespace-nowrap"
                  style={{
                    textShadow: "0 0 16px rgba(242,193,78,.35)",
                    // Long names would otherwise silently clip inside this
                    // banner's overflow-hidden border (invisible truncation,
                    // no ellipsis — e.g. "SHEFFIELD" rendering as just
                    // "IELD"). Scale the font down as the name gets longer
                    // so the FULL name always stays visible instead.
                    fontSize: `clamp(20px, ${Math.min(42, Math.round(760 / Math.max(6, player.name.length)))}px, 42px)`,
                  }}
                >
                  {player.name}
                </h2>
            ...
            {/* Info table */}
            <div data-wab-layer-id={`${side}-player-info`} className="mt-2 flex-1">
