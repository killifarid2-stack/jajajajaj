...
export async function listCategoryOverview(): Promise<CategoryOverviewGroup[]> {
  ...
  if (isSupabaseConfigured && tournamentIds.length > 0) {
    try {
      // NOTE: the `matches` table has no chung_club/hong_club columns (those
      // only exist on mat_live_status, the live-heartbeat table). Club data
      // for the archive comes from the `players` table query below instead —
      // selecting them here made this whole query fail against real Supabase
      // (400 from PostgREST), silently dropping match/status/player data too.
      const { data, error } = await supabase
        .from('matches')
        .select('id, tournament_id, status, chung_name, hong_name')
