...
// Full-coverage offline fallback: the flag-icons npm package (already a
// bundled dependency — see FlagImage.tsx) ships an SVG per ISO-3166 country,
// so every nationality resolves to a real local flag instead of falling
// through to blank/missing (previously only GB/KR/MA/TR — the 4 files
// manually placed in src/assets/flags — had a flag here at all).
const PACKAGE_FLAG_ASSETS = import.meta.glob('/node_modules/flag-icons/flags/4x3/*.svg', {
  eager: true,
  query: '?url',
  import: 'default',
}) as Record<string, string>;
...
export function getLocalFlagUrl(code: string): string {
  ...
  const key = `../assets/flags/${lower}.svg`;
  if (LOCAL_FLAG_ASSETS[key]) return LOCAL_FLAG_ASSETS[key];
  const pkgKey = `/node_modules/flag-icons/flags/4x3/${lower}.svg`;
  return PACKAGE_FLAG_ASSETS[pkgKey] || '';
}
