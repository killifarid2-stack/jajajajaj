...
export const announcer = {
  ...
  // call sequence.
  announceNextMatchAlert(args: { matNumber: number; matchNumber?: number; blue?: string; red?: string; lang?: 'ar' | 'en' | 'fr' }) {
    const isAr = args.lang === 'ar';
    const isFr = args.lang === 'fr';
    const parts = isAr
      ...
      : isFr
        ? [`Attention. Le match ${args.matchNumber ?? ''} est le prochain sur l’aire ${args.matNumber}.`,
           args.blue ? `${args.blue} contre ${args.red || ''}.` : '', 'Veuillez vous rendre sur l’aire.']
