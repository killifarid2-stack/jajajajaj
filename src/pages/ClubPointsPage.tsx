    ...
    <div className="min-h-screen gradient-dark">
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-8" dir={isAr ? 'rtl' : 'ltr'}>
        <div className="flex items-center gap-2">
          ...
          <h1 className="title-power text-2xl">{isAr ? 'نقاط الأندية والإنذارات' : isFr ? 'Points des clubs et pénalités' : 'Club Points & Warning Penalty'}</h1>
        </div>

        {/* ---- Live standings ---- */}
        <section className="panel p-4 md:p-6">
          <h2 className="font-display font-black text-lg mb-4 text-[hsl(var(--gold))]">
            {isAr ? 'ترتيب الأندية الحالي' : isFr ? 'Classement actuel des clubs' : 'Current Club Standings'}
          </h2>
          {standings.length === 0 ? (
            <p className="text-[hsl(var(--muted-foreground))] text-sm">
              {isAr ? 'لا توجد بيانات بعد — ستظهر النتائج بعد أول مباراة منتهية.' : isFr ? 'Aucune donnée — le classement apparaîtra après le premier match terminé.' : 'No data yet — standings appear after the first finished match.'}
            </p>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[hsl(var(--muted-foreground))] text-xs uppercase border-b border-white/10">
                    <th className="text-start py-2">{isAr ? 'النادي' : isFr ? 'Club' : 'Club'}</th>
                    <th className="text-center py-2">{isAr ? 'مباريات' : isFr ? 'Joués' : 'Played'}</th>
                    <th className="text-center py-2">{isAr ? 'فوز' : isFr ? 'Victoires' : 'Wins'}</th>
                    <th className="text-center py-2">{isAr ? 'نقاط الفوز' : isFr ? 'Pts victoire' : 'Win pts'}</th>
                    <th className="text-center py-2">🥇</th>
                    <th className="text-center py-2">🥈</th>
                    <th className="text-center py-2">🥉</th>
                    <th className="text-center py-2">{isAr ? 'نقاط الميداليات' : isFr ? 'Pts médailles' : 'Medal pts'}</th>
                    <th className="text-center py-2">{isAr ? 'خصومات' : isFr ? 'Pts pénalité' : 'Penalty pts'}</th>
                    <th className="text-center py-2 text-[hsl(var(--gold))]">{isAr ? 'المجموع' : isFr ? 'Total' : 'Total'}</th>
                  </tr>
                </thead>
                <tbody>
                  {standings.map((s) => (
        ...
        <section className="panel p-4 md:p-6">
          <h2 className="font-display font-black text-lg mb-4 text-[hsl(var(--gold))]">
            {isAr ? 'إعداد النقاط' : isFr ? 'Configuration des points' : 'Points Configuration'}
          </h2>
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-sm text-[hsl(var(--muted-foreground))]">{isAr ? 'نقاط لكل فوز' : isFr ? 'Points par victoire' : 'Points per win'}</label>
              <input
                type="number" min={0}
            ...
            <div className="flex items-center gap-2">
              <label className="text-sm text-[hsl(var(--gold))]">🥇 {isAr ? 'ذهبية' : isFr ? 'Or' : 'Gold'}</label>
              <input
                type="number" min={0}
            ...
            <div className="flex items-center gap-2">
              <label className="text-sm text-slate-300">🥈 {isAr ? 'فضية' : isFr ? 'Argent' : 'Silver'}</label>
              <input
                type="number" min={0}
            ...
            <div className="flex items-center gap-2">
              <label className="text-sm text-amber-600">🥉 {isAr ? 'برونزية' : isFr ? 'Bronze' : 'Bronze'}</label>
              <input
                type="number" min={0}
            ...
            <button onClick={handleSaveConfig} className="btn-power px-4 py-2 rounded-lg flex items-center gap-1.5 text-sm">
              <Save size={14} /> {configSaved ? (isAr ? 'تم الحفظ ✓' : isFr ? 'Enregistré ✓' : 'Saved ✓') : (isAr ? 'حفظ' : isFr ? 'Enregistrer' : 'Save')}
            </button>
          </div>
        ...
        <section className="panel p-4 md:p-6">
          <h2 className="font-display font-black text-lg mb-4 text-[hsl(var(--gold))]">
            {isAr ? 'أسباب الإنذار وقيمة الخصم' : isFr ? 'Motifs d’avertissement et pénalités' : 'Warning Reasons & Penalty Values'}
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-white/10 pt-3">
            ...
            <button onClick={handleAddReason} className="btn-power px-3 py-1.5 rounded-lg flex items-center gap-1 text-sm">
              <Plus size={14} /> {isAr ? 'إضافة' : isFr ? 'Ajouter' : 'Add'}
            </button>
          </div>
        </section>
        ...
        <section className="panel p-4 md:p-6">
          <h2 className="font-display font-black text-lg mb-4 flex items-center gap-2 text-[hsl(var(--gold))]">
            <AlertTriangle size={18} /> {isAr ? 'تسجيل إنذار' : isFr ? 'Enregistrer un avertissement' : 'Record a Warning'}
          </h2>
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <button onClick={handleRecordWarning} disabled={!warnClub.trim()}
              ...
              {isAr ? 'تسجيل' : isFr ? 'Enregistrer' : 'Record'}
            </button>
          </div>
