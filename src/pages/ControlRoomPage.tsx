...
import BroadcastOperationsPanel from '@/components/BroadcastOperationsPanel';
import {
      ...
      <div className="max-w-5xl mx-auto p-4 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h1 className="font-display text-lg font-bold text-[hsl(var(--gold))] flex items-center gap-2">
            <Radio size={20} />
            {isAr ? 'غرفة التحكم — حالة كل البسطات' : lang === 'fr' ? 'Salle de contrôle — Tous les tapis' : 'Control Room — All Mats'}
          </h1>
          <div className="flex items-center gap-2">
            <label className="text-xs text-[hsl(var(--muted-foreground))]">
              {isAr ? 'عدد البسطات' : lang === 'fr' ? 'Nombre de tapis' : 'Number of mats'}
            </label>
            ...
            <button onClick={refresh} className="p-2 rounded-lg bg-[hsl(var(--secondary))] text-[hsl(var(--muted-foreground))]" title={isAr ? 'تحديث' : lang === 'fr' ? 'Actualiser' : 'Refresh'}>
        ...
        <p className="text-xs text-[hsl(var(--muted-foreground))]">
          ...
          {lastRefresh && (
            <span className="ms-2 opacity-70">
              {isAr ? 'آخر تحديث:' : lang === 'fr' ? 'Dernière mise à jour :' : 'Last refresh:'} {lastRefresh.toLocaleTimeString(isAr ? 'ar-MA' : 'en-US')}
            </span>
          )}
        </p>
              ...
              <div key={matNumber} className="panel p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-display font-black text-[hsl(var(--gold))]">
                      {isAr ? 'بساط' : lang === 'fr' ? 'TAPIS' : 'MAT'} {String(matNumber).padStart(2, '0')}
                    </span>
                ...
                {s && (s.chung_name || s.hong_name) ? (
                  ...
                  <div className="text-xs text-[hsl(var(--muted-foreground))]">
                    {isAr ? 'لا توجد مباراة حاليًا' : lang === 'fr' ? 'Aucun match actuellement' : 'No match right now'}
                  </div>
                )}
                <button onClick={() => handleTakeControl(matNumber)}
                  ...
                  <Gavel size={12} /> {isAr ? 'تحكم بهذا البساط' : lang === 'fr' ? 'Contrôler ce tapis' : 'Control this mat'}
                </button>
              </div>
            );
          })}
