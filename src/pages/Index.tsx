...
export default function Index() {
  ...
  const handleSaveParEquipeMatch = () => {
    if (!isParEquipeMatch(state)) {
      toast.error(lang === 'ar' ? 'لا توجد مباراة Par Équipe نشطة للحفظ' : lang === 'fr' ? 'Aucun match Par Équipe actif à enregistrer' : 'No active Par Équipe match to save');
      return;
    }
    const saved = saveParEquipeMatch(state);
    saveParEquipeSafeSnapshot(state, 'referee_safe');
    if (saved) toast.success(lang === 'ar' ? '✓ تم حفظ مباراة Par Équipe' : lang === 'fr' ? '✓ Match Par Équipe enregistré' : '✓ Par Équipe match saved');
  };
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              <button
                ...
                title={lang === 'ar' ? 'إضافة مباراة Par Équipe' : lang === 'fr' ? 'Ajouter un match Par Équipe' : 'Add Par Équipe match'}
              >
  ...
  return (
    <div className="min-h-screen gradient-dark flex flex-col items-center justify-center p-6">
      ...
      <div className="absolute top-4 end-4 flex gap-2">
        <label className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[hsl(var(--gold))]/10 text-[hsl(var(--gold))] text-[hsl(var(--gold))] text-xs font-semibold border border-[hsl(var(--gold))]/20"><Globe size={12} /><select aria-label="Language" value={lang} onChange={e => setLang(e.target.value as 'en'|'ar'|'fr')} className="bg-transparent outline-none text-[hsl(var(--gold))]"><option value="ar">العربية</option><option value="en">English</option><option value="fr">Français</option></select></label>
      </div>
